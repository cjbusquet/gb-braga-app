...
export default function LoginPage() {
  ...
  return (
    <div style={{ height:'100vh', display:'flex', fontFamily:'var(--font-ui)', background:'#fff', overflow:'hidden' }}>
      ...
      {/* LEFT — Logo */}
      <div style={{ width:'44%', background:'#FFFFFF', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 48px', borderRight:'1px solid var(--border)', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', padding:'40px 52px', overflowY:'auto', background:'var(--bg-base)' }}>
        <div style={{ maxWidth:380 }}>
          ...
          <form onSubmit={go}>
            ...
            <button type="submit" disabled={loading} style={{ width:'100%', background:loading ? '#aaa' : GB.red, border:'none', borderRadius:'var(--radius-sm)', padding:'13px', color:'#fff', fontSize:13, fontWeight:700, fontFamily:'var(--font-display)', letterSpacing:'1px', textTransform:'uppercase', cursor:loading ? 'not-allowed' : 'pointer', boxShadow:'var(--shadow-red)' }}>
              {loading ? 'A entrar...' : 'Entrar'}
            </button>
          </form>

          {/* Demo buttons — ONLY shown when Supabase is NOT configured */}
          {!isConfigured && (
            <>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                {ROLES.map(r => {
                  ...
                  return (
                    <button key={r.email} onClick={() => quick(r.role, r.email)} style={{ display:'flex', alignItems:'center', gap:10, background: isActive ? rt.accentGlow : 'var(--bg-card)', border:`1px solid ${isActive ? rt.accent : 'var(--border)'}`, borderRadius:'var(--radius-sm)', padding:'10px 14px', cursor:'pointer', textAlign:'left', transition:'all 0.15s' }}>
                      ...
                      {isActive && <span style={{ color:rt.accent, fontSize:11 }}>✓</span>}
                    </button>
                  );
                })}
              </div>
          ...
          {isConfigured && (
            <div style={{ marginTop:20, textAlign:'center' }}>
              <a href="mailto:atendimento@gbbraga.com" style={{ color:'var(--text-muted)', fontSize:12 }}>
                Problemas a entrar? atendimento@gbbraga.com
              </a>
            </div>
          )}
