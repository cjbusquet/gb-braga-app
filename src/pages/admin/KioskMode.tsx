          ...
          <div className="flex flex-col flex-1 items-center max-w-[480px]">
              ...
              <>
                ...
                <div className="relative mb-6 w-60 h-60">
                  ...
                  <div className="absolute right-0 bottom-2 left-0 text-center">
                    <div className="text-[13px] font-bold tracking-[1px] text-gb-red">📍 GPS FENCE</div>
            ...
            {state === 'success' && lastCheckin && (
              <div className="text-center animate-[fadeIn_0.3s_ease]">
                <div className="flex justify-center items-center mx-auto mb-5 w-[100px] h-[100px] text-5xl rounded-full border-[3px] border-green-600/40 bg-green-600/[0.12]">✓</div>
                ...
                {lastCheckin.dist > 0 && (
                  <div className="text-xs text-green-600">📍 {lastCheckin.dist}m da academia · GPS confirmado</div>
                )}
                <div className="mt-2 text-sm font-bold text-green-600">OSS! 🥋</div>
              </div>
            )}

            {state === 'outside' && (
              <div className="max-w-[360px] text-center">
                <div className="flex justify-center items-center mx-auto mb-5 w-20 h-20 text-4xl rounded-full border-[3px] border-gb-red/30 bg-gb-red/10">📍</div>
        ...
        <div className={['flex flex-col w-full max-w-[480px]', showManual ? '' : 'lg:w-80'].join(' ')}>
          ...
          {showManual && (
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 Pesquisar..." autoFocus
              className="py-3 px-4 mb-3 w-full min-h-11 text-[15px] text-white rounded-[10px] border outline-none transition-colors duration-200 border-[#2A2A32] bg-[#161620] focus:border-gb-red focus-visible:ring-2 focus-visible:ring-gb-red/25"/>
          )}
            ...
            {(showManual ? filteredAlunos : alunos.filter(a => a.status === 'ativo').slice(0, 6)).map(a => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[13px] font-bold text-gb-red">📍 Check-in</span>
                </button>
              );
            })}
          </div>
